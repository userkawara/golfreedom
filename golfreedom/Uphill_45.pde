class Uphill_45{
  ...
  void hit_oblique_side(Ball b){
    ...
    if(x<ball_x2 && y<ball_y2 && ball_x2<x+size && ball_y2<y+size && ball_y2 - y > -(ball_x2 - x)+size){
      b.x = b.px;
      b.y = b.py;
      float buf = b.sx;
      b.sx = -b.sy * friction - b.out_loop;
      b.sy = -buf * friction;
      if(b.can_break){
        exist = false;
      }
  ...
  void hit_right_side(Ball b){
    if ((b.x - b.r) <= x + size && y < b.y && b.y < y+size && x+size < b.x){
      b.x = b.px;
      b.sx = -b.sx * rebound;
      b.sy = b.sy * friction;
      if(b.can_break){
  ...
  void hit_corner(Ball b){
    if(dist(b.x, b.y, x+size, y) < b.r){
      b.hit_corner(x+size, y, rebound);
      if(b.x < x+size){
        b.sx -= b.out_loop;
      }else{
        b.sx += b.out_loop;
